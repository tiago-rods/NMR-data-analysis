...
class JDXFormatter(FactoryProcessor):
    def process(self, jdx_data_list: List[Dict[str, Any]], experiment_names: List[str]) -> pd.DataFrame:
        ...
        first_data: Dict[str, Any] = jdx_data_list[0]
        ref_udic: Dict[str, Any] = ng.jcampdx.guess_udic(first_data["metadata"], first_data["spectral_data"])
        ref_uc: Dict[str, Any] = ng.fileiobase.uc_from_udic(ref_udic)
        ppm_scale: np.ndarray = ref_uc.ppm_scale()
        ...
        for data, exp_name in zip(jdx_data_list, experiment_names):
            intensities: np.ndarray = data["spectral_data"]
