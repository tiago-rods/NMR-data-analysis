...
class NmRanalysisFormatter(FactoryCSVFormatter):
    def format(self, file_path: str) -> Optional[str]:
        ...
        try:
            ...
            required_cols: list[str] = ['Sample', 'Metabolite', 'Quantity', 'Fitting Error']
            ...
            idx: pd.Index = df.groupby(['Sample', 'Base_Metabolite'])['Fitting Error'].idxmin()
            best_fits: pd.DataFrame = df.loc[idx]
            ...
            pivot_df: pd.DataFrame = best_fits.pivot(index='Base_Metabolite', columns='Sample', values='Quantity')
